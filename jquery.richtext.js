    ...
    $.fn.richText = function (options) {
        ...
        var $editor,
            ...
            $btnStrikethrough = $('<a />', {
                ...
                "title": settings.translations.strikethrough,
                html: '<span class="fa fa-strikethrough"></span>'
            }), // strikethrough
            $btnJustify = $('<a />', {
                class: "richText-btn",
            ...
            $btnMobileViewer = $('<a />', {
                class: "richText-btn",
                "title": 'Add mobile viewer',
                "data-command": "addMobileViewer",
        ...
        function init() {
            ...
            if (settings.strikethrough  === true) {
                $toolbarList.append($toolbarElement.clone().append($btnStrikethrough ));
            }
            ...
            /* Mobile Viewer */
            $toolbarList.append($toolbarElement.clone().append($btnMobileViewer));
            ...
            //var result = replaceParams($inputElement.val());
        ...
        $(document).on("paste", ".richText-editor", function () {
            console.log('Paste and clean ');
            //wait for paste event done
            setTimeout(function() {
                editeurRichText = $('#' + editorID).siblings('.richText-initial');
                editeurCodeSource = $('#' + editorID);
                console.log('editeurRichText.val():');
                console.log(editeurRichText.val());
                console.log(editeurRichText);
                editeurCodeSource.html(cleanWordTextStyle(editeurRichText.val()));
                updateTextarea();
                doSave(editorID);
            }, 2000);
        });
                ...
                if (!url) {
                    ...
                    if (settings.useSingleQuotes === true) {
                        if (align === "center") {
                            ...
                            html = "<table><tr><td><img src='" + url + "' align='" + align + "'></td></tr></table>";
                        }
                    } else {
                        if (align === "center") {
                            html = '<div style="text-align:center;"><img src="' + url + '"></div>';
        ...
        $(document).on("click", ".richText-toolbar a[data-command]", function (event) {
            ...
            if ($editor.length > 0 && id === editorID && (!$button.parent("li").attr('data-disable') || $button.parent("li").attr('data-disable') === "false")) {
                ...
                if (command === "addMobileViewer") {
                    $editor.append('<div class="iphone-bloc"><div class="iphone"><div class="border"><div class="responsive-html5-chat"></div></div></div>\n' +
                        '<table>\n' +
                        '  <tr>\n' +
                        '    <td>HB</td>\n' +
                        '    <td></td>\n' +
                        '    <td>Entrez vos messages ici</td>\n' +
                        '  </tr>\n' +
                        '  <tr>\n' +
                        '    <td>PUA</td>\n' +
                        '    <td></td>\n' +
        ...
        /**
         * Clean Microsoft Word text Mso tag
         */
        function cleanWordTextStyle(string) {
            console.log('Paste and clean : cleanWordTextStyle ');
            //MSO Word clean <!--MSO Style BLOCK
            if (string === undefined || string === null) {
                return '';
            }else{
                return string.replace(/<!--[\s\S]+?-->/g, "");// Word comments like conditional comments etc
            }
            //string = string.replace(/<(!|script[^>]*>.*?<\/script(?=[>\s])|\/?(\?xml(:\w+)?|img|meta|link|style|\w:\w+)(?=[\s\/>]))[^>]*>/g, "");// Remove comments, scripts (e.g., msoShowComment), XML tag, VML content, MS Office namespaced tags, and a few other tags
       ...
       */
        ...
        function toggleCode(editorID) {
            ...
            if ($editor.find('.richText-editor').is(":visible")) {
                ...
                //show select text part when no params exist
                convertCaretPosition(editorID, savedSelection);

                //remove alpha params contents
                $editor.find('.richText-initial').val(removeContentParams($editor.find('.richText-initial').val()));

                //use beautifier html before show
                const options = {
                    "indent_size": "4",
                    "indent_char": " ",
                ...
                const data= $editor.find('.richText-initial').val();
                var result = html_beautify(data, options);
                $editor.find('.richText-initial').val(result);
            } else {
                // show editor
                ...
                //replace alpha params with value
                var editor = $editor.find('.richText-editor');
                var content = editor.html();
                content = removeContentParams(content);//remove old content params
                content = replaceParams(content);
                editor.html(content);
        ...
        function getVideoCode(url, size) {
            ...
            if (video.platform === "YouTube") {
                ...
                html += '<iframe src="https://' + youtubeDomain + '/embed/' + video.id + '?ecver=2&amp;start=' + video.start + '" width="' + size[0] + '" height="' + size[1] + '" frameborder="0"' + (responsive === true ? ' style="position:absolute;width:100%;height:100%;left:0"' : '') + ' ' + allowfullscreen + '></iframe>';
                success = true;
         ...
         */
        var getUrlParameter = function getUrlParameter(sParam, str) {
            ...
            for (i = 0; i < sURLVariables.length; i++) {
                ...
                if (sParameterName[0] === sParam) {
                    if (sParameterName[1] === undefined){
                        ...
                        var uriPart = decodeURIComponent(sParameterName[1]);
                        //youtube
                        if (sParam === 't'){
        ...
        function getVideoID(url) {
            ...
            if (youtubeMatch && youtubeMatch[2].length === 11) {
                var start = getUrlParameter('t',url);
                return {
                    "platform": "YouTube",
                    "id": youtubeMatch[2],
                    "start" : start
